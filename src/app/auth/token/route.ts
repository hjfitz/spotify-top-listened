...
export async function GET(req: NextRequest) {
		...
		const { data } = await axios.post<RequestSuccess | RequestFailure>('https://accounts.spotify.com/api/token', body, {
				headers: {
		...
		if (!isSuccessfulResponse(data)) {
				return new Response(`Unable to authenticate: ${data.error_description}`, { status: 500 })
		}
